...
other list-shaped tools now return the same data twice, in two different shapes:
...
nothing about existing behavior changed.
...
agent reads `structuredContent[0].properties.componentId` directly — one field access, no
...
The saving is largest for tools an agent calls *in a loop* or *as input to another call* —
...
working exactly as before, and gets none of the savings. To actually use `structuredContent`,
