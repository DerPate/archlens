...
class WarModuleExtractionTest {
    ...
    static void extractModel() throws Exception {
        URL url = WarModuleExtractionTest.class.getClassLoader()
            .getResource("testprojects/war-modules-sample");
    ...
    void componentsExtractedFromInternalModules() {
        ...
        assertThat(model.components)
            .anyMatch(c -> "OrderService".equals(c.name));
        assertThat(model.components)
            .anyMatch(c -> "OrderRepository".equals(c.name));
    }
