...
public class TraceDataFlowTool {
    ...
    public String execute(JsonNode args) {
        try {
                ...
                paths = paths.stream()
                    .filter(p -> {
                        Entrypoint ep = model.entrypoints.stream()
                            .filter(e -> e.id.equals(p.entrypointId)).findFirst().orElse(null);
                        return ep != null && (ep.name.toLowerCase().contains(lower)
            ...
            if (paramFilter != null) {
                paths = paths.stream()
                    .filter(p -> p.trackedParam.equals(paramFilter)
    ...
    private String format(List<DataFlowPath> paths, ArchitectureModel model) {
        ...
        for (DataFlowPath path : paths) {
            Entrypoint ep = model.entrypoints.stream()
                .filter(e -> e.id.equals(path.entrypointId)).findFirst().orElse(null);

            String epLabel = ep != null
                ? (ep.httpMethod != null ? ep.httpMethod + " " : "") + (ep.path != null ? ep.path : ep.name)
                : path.entrypointId;

            sb.append("## ").append(epLabel).append(" → param: ").append(path.trackedParam).append("\n");
            sb.append("  id: ").append(path.id).append("\n");

            if (!path.steps.isEmpty()) {
                sb.append("  flow:\n");
                for (DataFlowStep step : path.steps) {
                    sb.append("    ").append(step.index + 1).append(". ")
                      .append(step.componentName).append(".").append(step.method)
                      .append(" (as '").append(step.localName).append("')\n");
                }
            }
            ...
            if (!path.sinks.isEmpty()) {
                ...
                for (DataFlowSink sink : path.sinks) {
                    ...
                    if (sink.source != null && sink.source.file != null
                        ...
                        sb.append("  (").append(slash >= 0 ? file.substring(slash + 1) : file)
                          .append(":").append(sink.source.line).append(")");
                    }
                    sb.append("\n");
                }
            }
