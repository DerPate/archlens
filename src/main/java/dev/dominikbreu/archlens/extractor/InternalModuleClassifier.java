    ...
    public String classify(Collection<CtType<?>> types, AppEntry app) {
        ...
        long archTypes = types.stream()
                .filter(t -> t.getAnnotations().stream()
                        .anyMatch(a -> ARCHITECTURAL_ANNOTATION_SIMPLE_NAMES.contains(
        ...
        if (nameLower.contains("util")
                || nameLower.contains("dto")
                || nameLower.contains("common")
                || nameLower.contains("api")
                || nameLower.contains("base")
                || nameLower.contains("shared")) {
            score -= 1;
        }

        // +1 if name suggests domain/business module
        if (nameLower.contains("service")
                || nameLower.contains("domain")
