...
import dev.dominikbreu.archlens.workflow.WorkflowGraph;
import dev.dominikbreu.archlens.workflow.WorkflowGraphBuilder;
        ...
        for (DataFlowSink sink : current.sinks) {
            if (sink.linkedPathIds == null
                    || !sink.linkedPathIds.contains(
                            dev.dominikbreu.archlens.model.ids.DataFlowPathId.deserialize(link.toPathId()))) continue;
