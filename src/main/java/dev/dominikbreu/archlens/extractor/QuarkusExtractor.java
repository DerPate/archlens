...
public class QuarkusExtractor {
    ...
    private static final Set<String> SCHEDULED_ANNOTATIONS =
            Set.of("io.quarkus.scheduler.Scheduled", "javax.ejb.Schedule", "jakarta.ejb.Schedule");

    private static final Set<String> ENTITY_ANNOTATIONS =
            Set.of("javax.persistence.Entity", "jakarta.persistence.Entity");

    private static final Set<String> INCOMING_ANNOTATIONS =
            Set.of("org.eclipse.microprofile.reactive.messaging.Incoming");

    private static final Set<String> OUTGOING_ANNOTATIONS =
            Set.of("org.eclipse.microprofile.reactive.messaging.Outgoing");
    ...
    private static final Set<String> WS_ENDPOINT_ANNOTATIONS =
            Set.of("javax.websocket.server.ServerEndpoint", "jakarta.websocket.server.ServerEndpoint");
    ...
    private static final Set<String> KAFKA_PRODUCER_TYPES =
            Set.of("org.apache.kafka.clients.producer.KafkaProducer", "org.apache.kafka.clients.producer.Producer");

    private static final Set<String> KAFKA_CONSUMER_TYPES =
            Set.of("org.apache.kafka.clients.consumer.KafkaConsumer", "org.apache.kafka.clients.consumer.Consumer");
    ...
    private void extractRawClientInterfaces(CtType<?> type, Component component, ArchitectureModel model) {
        ...
        for (CtField<?> field : type.getFields()) {
            ...
            MessagingCallSiteResolver.Role roleHint =
                    switch (kind.role) {
            ...
            trackedFields.put(field.getSimpleName(), new MessagingCallSiteResolver.TrackedField(kind.broker, roleHint));
        }
        ...
        for (MessagingCallSiteResolver.Finding f : callSiteResolver.resolve(type, trackedFields)) {
            findingsByField
                    .computeIfAbsent(f.fieldName(), k -> new ArrayList<>())
                    .add(f);
        }
    ...
    private void addRawClientInterface(
        ...
        entry.broker = broker;
        entry.source =
                new SourceInfo(getFile(field), getLine(field), evidence, "call-site".equals(evidence) ? 0.95 : 0.85);
    ...
    private void emitGrpcEntrypoints(CtType<?> type, Component component, ArchitectureModel model) {
        for (CtMethod<?> method : type.getMethods()) {
            ...
            dev.dominikbreu.archlens.model.ids.EntrypointId epId = new dev.dominikbreu.archlens.model.ids.EntrypointId(
                    dev.dominikbreu.archlens.model.ids.ComponentId.of(type.getQualifiedName()),
                    method.getSimpleName(),
                    "grpc");
