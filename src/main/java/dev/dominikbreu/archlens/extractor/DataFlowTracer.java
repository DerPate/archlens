...
public class DataFlowTracer {
    ...
    private void traceEntrypoint(Entrypoint ep, ModelIndex index, List<DataFlowPath> result) {
        ...
        for (String tracked : trackedNames) {
            ...
            path.id = dev.dominikbreu.archlens.model.ids.DataFlowPathId.of(ep.id, tracked);
    ...
    private void linkStoreSinksToFieldReaders(List<DataFlowPath> paths, ArchitectureModel model, ModelIndex index) {
        ...
        Map<dev.dominikbreu.archlens.model.ids.FieldRef, List<dev.dominikbreu.archlens.model.ids.DataFlowPathId>>
                readerPathsByKey = new HashMap<>();
        for (DataFlowPath p : paths) {
            Set<dev.dominikbreu.archlens.model.ids.FieldRef> keys = readsByEntrypoint.get(p.entrypointId);
            if (keys == null) continue;
            for (dev.dominikbreu.archlens.model.ids.FieldRef key : keys) {
        ...
        Map<dev.dominikbreu.archlens.model.ids.DataFlowPathId, String> pathIdToEpId = new HashMap<>();
    ...
    private void linkStoreSink(
            ...
            Map<dev.dominikbreu.archlens.model.ids.FieldRef, List<dev.dominikbreu.archlens.model.ids.DataFlowPathId>>
                    readerPathsByKey,
            Map<dev.dominikbreu.archlens.model.ids.DataFlowPathId, String> pathIdToEpId) {
        ...
        dev.dominikbreu.archlens.model.ids.FieldRef key =
                new dev.dominikbreu.archlens.model.ids.FieldRef(s.fieldOwnerComponentId, s.fieldName);
        List<dev.dominikbreu.archlens.model.ids.DataFlowPathId> readerIds = readerPathsByKey.get(key);
        if (readerIds == null) return;
        for (dev.dominikbreu.archlens.model.ids.DataFlowPathId rid : readerIds) {
    ...
    private void linkMessagingSinksToChannelConsumers(List<DataFlowPath> paths, ArchitectureModel model) {
        Map<String, List<dev.dominikbreu.archlens.model.ids.DataFlowPathId>> consumerPathsByDestination =
                new HashMap<>();
        ...
        Map<String, List<dev.dominikbreu.archlens.model.ids.DataFlowPathId>> consumerPathsByChannel = new HashMap<>();
    ...
    private void indexConsumerPath(
            ...
            Map<String, List<dev.dominikbreu.archlens.model.ids.DataFlowPathId>> consumerPathsByDestination,
            Map<String, List<dev.dominikbreu.archlens.model.ids.DataFlowPathId>> consumerPathsByChannel) {
    ...
    private void linkMessagingSink(
        ...
        List<dev.dominikbreu.archlens.model.ids.DataFlowPathId> candidates =
                consumerPathsByDestination.getOrDefault(key, List.of());
        ...
        for (dev.dominikbreu.archlens.model.ids.DataFlowPathId targetPathId : candidates) {
    ...
    private Set<dev.dominikbreu.archlens.model.ids.FieldRef> collectReachableReadFieldKeys(
        ...
        Set<dev.dominikbreu.archlens.model.ids.MethodRef> seen = new HashSet<>();
        dev.dominikbreu.archlens.model.ids.MethodRef start =
                new dev.dominikbreu.archlens.model.ids.MethodRef(ep.componentId, ep.name);
        stack.push(start);
        seen.add(start);
        int budget = 64;
        while (!stack.isEmpty() && budget-- > 0) {
            dev.dominikbreu.archlens.model.ids.MethodRef current = stack.pop();
            for (FieldAccess r : index.fieldAccess.reads(current.component(), current.method())) {
                if (r.fieldBinding instanceof dev.dominikbreu.archlens.model.ids.FieldBinding.CrossComponent(var ref)) {
                    keys.add(ref);
                }
            }
    ...
    private void recordFieldWriteSinks(
        ...
        for (FieldAccess fw : ctx.index().fieldAccess.writes(compId, method)) {
            for (Map.Entry<String, String> e : currentToOriginal.entrySet()) {
                ...
                dev.dominikbreu.archlens.model.ids.ComponentId fieldOwner = fw.fieldBinding
                                instanceof dev.dominikbreu.archlens.model.ids.FieldBinding.CrossComponent(var ref)
                        ? ref.owner()
                        : fw.componentId;
    ...
    private void linkPersistenceWritesToReaders(List<DataFlowPath> paths, ArchitectureModel model) {
        ...
        Map<String, List<dev.dominikbreu.archlens.model.ids.DataFlowPathId>> readPathsByEntity =
                indexPersistenceReadPaths(paths, entrypointById);
        for (DataFlowPath path : paths) {
            for (DataFlowSink sink : path.sinks) {
    ...
    private void linkPersistenceWriteSink(
        ...
        for (dev.dominikbreu.archlens.model.ids.DataFlowPathId targetPathId :
                readPathsByEntity.getOrDefault(sink.entityType, List.of())) {
            if (!targetPathId.equals(path.id) && !sink.linkedPathIds.contains(targetPathId)) {
                sink.linkedPathIds.add(targetPathId);
