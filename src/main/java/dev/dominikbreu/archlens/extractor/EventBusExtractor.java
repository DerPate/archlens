...
public class EventBusExtractor {
    ...
    public void extract(Collection<CtType<?>> types, ArchitectureModel model, AppId appId) {
        Set<dev.dominikbreu.archlens.model.ids.ComponentId> existingIds =
    ...
    public void linkCrossModuleEvents(ArchitectureModel model) {
        Set<dev.dominikbreu.archlens.model.ids.DependencyId> existingDepIds =
        ...
        for (Map.Entry<String, List<String>> entry : producersByEventType.entrySet()) {
            ...
            for (String producerId : producers) {
                for (String consumerId : consumers) {
                    ...
                    dev.dominikbreu.archlens.model.ids.DependencyId depId =
                            new dev.dominikbreu.archlens.model.ids.DependencyId(
                                    producerId + "->cdi-event:" + eventType + "->" + consumerId);
                    if (existingDepIds.add(depId)) {
                        Dependency dep = new Dependency();
                        dep.id = depId;
                        dep.fromId = dev.dominikbreu.archlens.model.ids.ComponentId.of(producerId);
                        dep.toId = dev.dominikbreu.archlens.model.ids.ComponentId.of(consumerId);
                        dep.kind = "cdi-event";
